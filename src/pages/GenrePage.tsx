...
import { discover, getLanguages, type Movie } from "@/lib/tmdb";
...
export default function GenrePage() {
  ...
  const load = useCallback(async (p: number, reset = false) => {
    ...
    setLoading(true);
    try {
      const r = await discover(type as "movie" | "tv", {
        page: p,
        with_genres: id,
        sort_by: sortFilter,
        with_original_language: languageFilter || settings.language || undefined,
      });
      setItems(prev => reset ? r.results : [...prev, ...r.results]);
      setHasMore(r.page < r.total_pages);
    } finally { setLoading(false); }
  }, [type, id, languageFilter, settings.language, sortFilter]);
  ...
  return (
    <div className="px-4 sm:px-8 py-6 pb-20">
      <h1 className="text-2xl font-bold text-foreground mb-4">{name} - {type === "movie" ? "Movies" : "TV Shows"}</h1>
      <div className="grid grid-cols-2 gap-3 mb-6 rounded-xl border border-border/40 bg-card p-4">
        ...
        <label className="text-xs uppercase tracking-wider text-muted-foreground">Popularity
