...
export default function PostCard({ item, type, slider = false }: Props) {
  ...
  return (
    <Link to={`/${mediaType}/${item.id}`} className={wrapperClass} aria-label={title}>
      <div className="relative aspect-video rounded-[4px] overflow-hidden bg-muted/40 ring-1 ring-white/10 shadow-[0_8px_30px_-8px_hsl(240_30%_2%/0.6)] transition-all duration-500 group-hover:ring-primary/70 group-hover:shadow-[0_18px_50px_-10px_hsl(265_90%_50%/0.55)] group-hover:-translate-y-1">
        <img
          src={imgSrc}
          alt={title}
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center shadow-2xl shadow-primary/50 scale-90 group-hover:scale-100 transition-transform ring-1 ring-white/30">
